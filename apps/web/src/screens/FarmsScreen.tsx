          ...
          <tbody>
            {props.farms.map((farm) => (
              <tr key={farm.id}>
                ...
                <td>
                  <Actions>
                    <IconButton type="button" title="Editar fazenda" disabled={props.savingFarmId === farm.id} onClick={() => props.onSetFarmDraft({ id: farm.id, name: farm.name, city: farm.city, state: farm.state, totalArea: Number(farm.totalArea), arableArea: Number(farm.arableArea), vegetationArea: Number(farm.vegetationArea) })}>
                      <Edit3 size={16} />
                    </IconButton>
